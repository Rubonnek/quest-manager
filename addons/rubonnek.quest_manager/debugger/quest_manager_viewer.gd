...
@export var quest_manager_viewer_quest_entries_is_active_state_check_box_ : CheckBox
@export var quest_manager_viewer_quest_entries_is_accepted_state_check_box_ : CheckBox
@export var quest_manager_viewer_quest_entries_is_completed_state_check_box_ : CheckBox
...
func _ready() -> void:
	...
	quest_manager_viewer_quest_entries_is_active_state_check_box_.toggled.connect(__force_original_is_active_state)
	quest_manager_viewer_quest_entries_is_accepted_state_check_box_.toggled.connect(__force_original_is_accepted_state)
	quest_manager_viewer_quest_entries_is_completed_state_check_box_.toggled.connect(__force_original_is_completed_state)
...
func __on_session_started() -> void:
	...
	# Clear the quest tree
	quest_manager_viewer_quest_entries_tree_.clear()

	# Clear the state view
	quest_manager_viewer_quest_entries_is_active_state_check_box_.set_pressed(false)
	quest_manager_viewer_quest_entries_is_accepted_state_check_box_.set_pressed(false)
	quest_manager_viewer_quest_entries_is_completed_state_check_box_.set_pressed(false)
	quest_manager_viewer_quest_entries_is_active_state_check_box_.set_disabled(true)
	quest_manager_viewer_quest_entries_is_accepted_state_check_box_.set_disabled(true)
	quest_manager_viewer_quest_entries_is_completed_state_check_box_.set_disabled(true)
...
func __on_session_stopped() -> void:
	if not is_instance_valid(quest_manager_viewer_manager_selection_tree_.get_root()) or quest_manager_viewer_manager_selection_tree_.get_root().get_child_count() == 0:
		quest_manager_viewer_quest_entries_view_warning_label_.set_text(_m_original_quest_view_warning_text)
	...
	for topmost_quest_id : int in quest_manager.size():
		...
		if not quest_entry.has_parent():
			...
			while not quest_id_stack.is_empty():
				...
				var acceptance_count : int = quest.get_acceptance_count()
				var completion_count : int = quest.get_completion_count()
				var tooltip_string : String = "ID: %d\nAcceptance Count: %d\nCompletion Count: %d\nDescription: %s" % [quest_id, acceptance_count, completion_count, quest_description]
				quest_tree_item.set_tooltip_text(column, tooltip_string)

				var quest_state_data : Dictionary = {}
				if quest.has_metadata():
					quest_state_data["quest_metadata"] = quest.get_metadata_data()
				quest_state_data["is_active"] = quest.is_active()
				quest_state_data["is_accepted"] = quest.is_accepted()
				quest_state_data["is_completed"] = quest.is_completed()
				quest_tree_item.set_metadata(column, quest_state_data)
...
func __on_quest_view_selection_item_selected() -> void:
	...
	if is_instance_valid(selected_tree_item):
		var column : int = 0
		var quest_state_data : Dictionary = selected_tree_item.get_metadata(column)

		# Update the state view
		quest_manager_viewer_quest_entries_is_active_state_check_box_.set_disabled(false)
		quest_manager_viewer_quest_entries_is_accepted_state_check_box_.set_disabled(false)
		quest_manager_viewer_quest_entries_is_completed_state_check_box_.set_disabled(false)
		var is_active : bool = quest_state_data["is_active"]
		var is_accepted : bool = quest_state_data["is_accepted"]
		var is_completed : bool = quest_state_data["is_completed"]
		quest_manager_viewer_quest_entries_is_active_state_check_box_.set_pressed(is_active)
		quest_manager_viewer_quest_entries_is_accepted_state_check_box_.set_pressed(is_accepted)
		quest_manager_viewer_quest_entries_is_completed_state_check_box_.set_pressed(is_completed)

		# Update the metadata view
		if not quest_state_data.has("metadata"):
...
func __force_original_is_active_state(_p_pressed : bool) -> void:
	var selected_tree_item : TreeItem = quest_manager_viewer_quest_entries_tree_.get_selected()
	if is_instance_valid(selected_tree_item):
		var column : int = 0
		var quest_state_data : Dictionary = selected_tree_item.get_metadata(column)
...
func __force_original_is_accepted_state(_p_pressed : bool) -> void:
	var selected_tree_item : TreeItem = quest_manager_viewer_quest_entries_tree_.get_selected()
	if is_instance_valid(selected_tree_item):
		var column : int = 0
		var quest_state_data : Dictionary = selected_tree_item.get_metadata(column)
...
func __force_original_is_completed_state(_p_pressed : bool) -> void:
	var selected_tree_item : TreeItem = quest_manager_viewer_quest_entries_tree_.get_selected()
	if is_instance_valid(selected_tree_item):
		var column : int = 0
		var quest_state_data : Dictionary = selected_tree_item.get_metadata(column)
